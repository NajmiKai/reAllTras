...
$submenuOpen = in_array($currentPage, ['permohonanPengguna.php', 'permohonanIbuPejabat.php', 'permohonanDikuiri.php']);
   ...
   $_SESSION['LAST_ACTIVITY'] = time();
    ...
    if ($application_data) {
        ...
        $sql = "SELECT status, pbr_csm2_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($status, $pbr_csm2_id);
        if ($stmt->fetch()) {
            if ($status !== 'Menunggu pengesahan PBR2 CSM' || $pbr_csm2_id !== null) {
                $isApproved = true;
            }
        }
        $stmt->close();
...
<div class="main-container">
   ...
   <div class="sidebar" id="sidebar">
        ...
        <div id="wilayahSubmenu" class="submenu" style="display: <?= $submenuOpen ? 'block' : 'none' ?>;">
            ...
            <a href="permohonanIbuPejabat.php" class="active">Buku Rekod</a>
        ...
        <form action="send_mail2.php" method="POST" enctype="multipart/form-data">            
            ...
            <div class="card-body row">
                    <div class="form-check" style="margin-left: 10px;">
                        <input class="form-check-input" type="checkbox" id="buku_rekod" name="buku_rekod" required <?php if ($isApproved) echo 'checked disabled'; ?>>
