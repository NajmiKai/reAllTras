...
$submenuOpen = in_array($currentPage, ['permohonanPengguna.php', 'permohonanIbuPejabat.php', 'permohonanDikuiri.php']);
  ...
  // Set session timeout duration (in seconds)
    ...
    if ($application_data) {
        ...
        $sql = "SELECT status, pbr_csm1_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($status, $pbr_csm1_id);
        if ($stmt->fetch()) {
            if ($status !== 'Menunggu pengesahan PBR CSM' || $pbr_csm1_id !== null) {
                $isApproved = true;
            }
        }
        $stmt->close();
        ...
        <form action="send_mail.php" method="POST" enctype="multipart/form-data" id="permohonanForm" onsubmit="return handleFormSubmit()">            
            ...
            <div class="card-body row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="file" name="dokumen[]" multiple <?php if ($isApproved) echo 'disabled'; ?>>
                </div>
                <div class="col-md-6 mb-3">
