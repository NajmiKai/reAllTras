...
function countByStatus($conn, $table, $admin_id, $status = 'total') {
    if ($status === 'total') {
        ...
        $query = "SELECT COUNT(*) AS jumlah FROM $table WHERE pengesah_HQ_id = ? OR status = 'Menunggu pengesahan pengesah HQ'";
        ...
        $query = "SELECT COUNT(*) AS jumlah FROM $table WHERE pengesah_HQ_id = ? ";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $admin_id);
    } else {
        return 0;
