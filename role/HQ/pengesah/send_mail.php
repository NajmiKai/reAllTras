...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $stmt_wilayah = $conn->prepare("UPDATE wilayah_asal SET status = ?, ulasan_pengesah_HQ = ?, pengesah_HQ_id = ?, tarikh_keputusan_pengesah_HQ = ? WHERE id = ?");
        $stmt_wilayah->bind_param("ssssi", $status, $ulasan, $admin_id, $tarikh_keputusan, $wilayah_asal_id);
        $stmt_wilayah->execute();
        
        // Log the status change
        logApplicationStatusChange($conn, 'admin', $admin_id, $wilayah_asal_id, $old_status, $status, "Pengesah HQ updated application status");
        
        $stmt_wilayah->close();

         //insert into document_logs
         $original_status_permohonan = $_POST['status_permohonan'] ?? '';
         $tindakan = ($original_status_permohonan === 'tidak disahkan') ? "Dikuiri" : "Disahkan";
         $ulasan = $_POST['ulasan'] ?? "-";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($data = $result->fetch_assoc()) {
                ...
                try {
                    ...
                    $mail->isHTML(true);
                    $mail->Subject = 'Permohonan Tambang Ziarah Wilayah (TZW) : Pertimbangan Kelulusan Oleh Ketua Jabatan';
                    $mail->Body = "
                        ...
                        <p>Permohonan Tambang Ziarah Wilayah (TZW) oleh pegawai telah disemak. Mohon pertimbangan serta kelulusan tuan/puan.</p>
        
                        <p>Sila klik pautan/butang di bawah untuk tindakan lanjut dan maklumat permohonan.</p>
        
                        <p><a href='" . getFullUrl("role/HQ/pelulus/viewdetails.php?kp=$kp") . "'><b><u>PAPAR MAKLUMAT PERMOHONAN</u></b></a></p><br>
