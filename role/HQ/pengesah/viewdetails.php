...
$redirect_back = $_SERVER['HTTP_REFERER'] ?? 'dashboard.php';
    ...
    if ($application_data) {
        ...
        $sql = "SELECT status, pengesah_HQ_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($status, $pengesah_HQ_id);
        if ($stmt->fetch()) {
            if ($status !== 'Menunggu pengesahan pengesah HQ' || $pengesah_HQ_id !== null) {
                $isApproved = true;
            }
        }
        $stmt->close();
            ...
            <div class="col-md-6">
                ...
                <p class="form-control-static ps-2"><?= $penyemak_name ?></p>                    
                    ...
                    <select class="form-select" name="status_permohonan" id="status_select" required onchange="toggleUlasan()" <?php if ($isApproved) echo 'disabled'; ?>>
                        ...
                        <option value="disahkan">Permohonan Disahkan</option>
                        <option value="tidak disahkan">Permohonan Tidak Disahkan</option>
                    </select>
                </div>
