...
$redirect_back = $_SERVER['HTTP_REFERER'] ?? 'dashboard.php';
    ...
    if ($application_data) {
        ...
        $sql = "SELECT status, penyemak_HQ2_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($status, $penyemak_HQ2_id);
        if ($stmt->fetch()) {
            if ($status !== 'Menunggu pengesahan penyemak2 HQ' || $penyemak_HQ2_id !== null) {
                $isApproved = true;
            }
        }
        $stmt->close();
