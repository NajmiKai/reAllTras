...
function countByStatus($conn, $table, $admin_id, $status = 'total') {
    if ($status === 'total') {
        ...
        $query = "SELECT COUNT(*) AS jumlah FROM $table WHERE penyediaKemudahan_kewangan_id = ? OR status = 'Menunggu pengesahan penyedia kemudahan kewangan'";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $admin_id);
    } elseif ($status === 'Sedang diproses') {
        $query = "SELECT COUNT(*) AS jumlah FROM $table WHERE status = 'Menunggu pengesahan penyedia kemudahan kewangan'";
        $stmt = $conn->prepare($query);
    } elseif ($status === 'Berjaya diproses') {
        $query = "SELECT COUNT(*) AS jumlah FROM $table WHERE penyediaKemudahan_kewangan_id = ? ";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $admin_id);
    } else {
        return 0;
