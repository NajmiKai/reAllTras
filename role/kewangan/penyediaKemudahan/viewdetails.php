...
include '../../../connection.php';
    ...
    if ($application_data) {
        ...
        $sql = "SELECT penyediaKemudahan_kewangan_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($penyediaKemudahan_kewangan_id);
        if ($stmt->fetch()) {
                if ($penyediaKemudahan_kewangan_id === $admin_id || $penyediaKemudahan_kewangan_id !== null) { 
                $isApproved = true;
            }
        }
        $stmt->close();
  ...
  <div class="col p-4" style="margin-left: 250px;">
        ...
        <?php
         ...
         ?>     
            ...
            <!-- <input type="hidden" name="userKP" value="<?= $application_data['kp'] ?>"> -->
