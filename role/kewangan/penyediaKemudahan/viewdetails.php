...
$redirect_back = $_SERVER['HTTP_REFERER'] ?? 'dashboard.php';
    ...
    if ($application_data) {
        ...
        $sql = "SELECT status, penyediaKemudahan_kewangan_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($status, $penyediaKemudahan_kewangan_id);
        if ($stmt->fetch()) {
            if ($status !== 'Menunggu pengesahan penyedia kemudahan kewangan' || $penyediaKemudahan_kewangan_id !== null) {
                $isApproved = true;
            }
        }
        $stmt->close();
