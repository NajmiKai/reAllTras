...
function getApprovedCounts($conn) {
    ...
    $query_selesai = "SELECT COUNT(*) as count FROM wilayah_asal JOIN user ON user.kp = wilayah_asal.user_kp 
                      WHERE status NOT IN ('Menunggu pengesahan penyedia kemudahan kewangan') 
...
function getStatusFilter($status, $approved_status = 'dalam_proses') {
    switch ($status) {
        case 'processing':
            return "status = 'Menunggu pengesahan penyedia kemudahan kewangan'";
        case 'approved':
            $selesai = "status NOT IN ('Menunggu pengesahan penyedia kemudahan kewangan') AND status_permohonan = 'Selesai'";
            $not_selesai = "status NOT IN ('Menunggu pengesahan penyedia kemudahan kewangan') AND status_permohonan != 'Selesai'";
