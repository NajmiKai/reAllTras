...
$redirect_back = $_SERVER['HTTP_REFERER'] ?? 'dashboard.php';
   ...
   $_SESSION['LAST_ACTIVITY'] = time();
    ...
    if ($application_data) {
        ...
        $sql = "SELECT status, pengesah_kewangan_id FROM wilayah_asal WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $wilayah_asal_id);
        $stmt->execute();
        $stmt->bind_result($status, $pengesah_kewangan_id);
        if ($stmt->fetch()) {
            if ($status !== 'Menunggu pengesahan pengesah kewangan' || $pengesah_kewangan_id !== null) {
                $isApproved = true;
            }
        }
        $stmt->close();
        ...
        <form action="send_mail.php" method="POST" enctype="multipart/form-data" id="permohonanForm" onsubmit="return handleFormSubmit()">            
            ...
            <div class="card-body row">
                <div class="col-md-6 mb-3">
                    <label for="status_select" class="form-label">Disahkan baki</label>
                    <select class="form-select" name="status_permohonan" id="status_select" required onchange="toggleUlasan()" <?php if ($isApproved) echo 'disabled'; ?>>
                        ...
                        <option value="mencukupi">Peruntukan mencukupi</option>
                        <option value="tidak mencukupi">Peruntukan tidak mencukupi</option>
                    </select>
                </div>
...
document.querySelector('.toggle-sidebar').addEventListener('click', function (e) {
    ...
    if (select.value === 'tidak mencukupi') {
    ...
    function handleFormSubmit() {
            ...
            if (status === 'tidak mencukupi') {
                form.action = 'send_mail3.php';
            } else if (status === 'mencukupi') {
