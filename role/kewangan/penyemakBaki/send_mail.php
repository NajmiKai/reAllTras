...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $baki_kewangan = $_POST['baki_kewangan'];
        ...
        $stmt_wilayah = $conn->prepare("UPDATE wilayah_asal SET status = ?, ulasan_penyemakBaki_kewangan = ?, penyemakBaki_kewangan_id = ?, tarikh_keputusan_penyemakBaki_kewangan = ? WHERE id = ?");
        $stmt_wilayah->bind_param("ssisi", $status, $baki_kewangan, $admin_id, $tarikh_keputusan, $wilayah_asal_id);
        $stmt_wilayah->execute();
        $stmt_wilayah->close();


        $sql = "SELECT * FROM admin WHERE role = 'Pengesah Kewangan'";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($data = $result->fetch_assoc()) {
                ...
                try {
                    ...
                    $mail->isHTML(true);
                    $mail->Subject = 'Permohonan Tambang Ziarah Wilayah (TZW) : Pengesahan Peruntukan Kewangan';
                    $mail->Body = "
                        ...
                        <p><a href='http://localhost/reAllTras/role/kewangan/pengesah/viewdetails.php?kp=$kp'><b><u>PAPAR MAKLUMAT PERMOHONAN</u></b></a></p><br>
